...
- New column: **Price vs Support %** = `((Current Price - Support) / Support) * 100`
- **Price vs Support % < 0.3** is highlighted in deep green to mark a near-support buy opportunity
...
- Filename includes active RSI config for traceability, e.g.:  
      `market-report-2026-04-29_14-24_RSI30-40_OB70.xlsx`
...
### Run all reports (Sharemarket + SMC + Crypto)

```powershell
.\run-all-reports.ps1
```

This orchestration script now runs three services and creates one combined output folder under `combined-reports` with:
- `sharemarket-report.xlsx`
- `smc-report.xlsx`
- `crypto-report.xlsx`

The crypto report is generated by the separate `crypto-buy-signals-service` via its REST endpoint.
It now evaluates the configured crypto and stock universe and prints a crypto summary line with `BUY`, `WAIT`, `SELL_WATCH`, `NO_DATA`, and `INSUFFICIENT_DATA` counts during the run.
...
| Property | Default | Description |
|----------|---------|-------------|
| `market.data-interval` | `4h` | Bar interval (`1h`, `4h`, `1d`, `1wk`, `1mo`) |
